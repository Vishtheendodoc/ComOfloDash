...
with col_stats:
    if not agg_df.empty:
        ...
        <div class="market-stats">
            <h3 style="margin-top: 0; color: #2c3e50;">Market Stats</h3>
            
            <div class="stat-item">
                <div class="stat-label">Open</div>
                <div class="stat-value" style="color: #6c757d;">{latest.get('open', 0):.1f}</div>
            </div>
            
            <div class="stat-item">
                <div class="stat-label">High</div>
                <div class="stat-value" style="color: #27ae60;">{latest.get('high', 0):.1f}</div>
            </div>
            
            <div class="stat-item">
                <div class="stat-label">Low</div>
                <div class="stat-value" style="color: #e74c3c;">{latest.get('low', 0):.1f}</div>
            </div>
            
            <div class="stat-item">
                <div class="stat-label">Close</div>
                <div class="stat-value" style="color: #6c757d;">{latest.get('close', 0):.1f}</div>
            </div>
            
            <div class="stat-item">
                <div class="stat-label">Buy Volume</div>
                <div class="stat-value" style="color: #27ae60;">{latest.get('buy_volume', 0):,.0f}</div>
            </div>
            
            <div class="stat-item">
                <div class="stat-label">Sell Volume</div>
                <div class="stat-value" style="color: #e74c3c;">{latest.get('sell_volume', 0):,.0f}</div>
            </div>
            
            <div class="stat-item">
                <div class="stat-label">Cumulative Delta</div>
                <div class="stat-value" style="color: {'#27ae60' if latest.get('cumulative_tick_delta', 0) > 0 else '#e74c3c'};">{latest.get('cumulative_tick_delta', 0):+.0f}</div>
            </div>
            
            <div class="stat-item">
                <div class="stat-label">Volume Delta</div>
                <div class="stat-value" style="color: {'#27ae60' if latest.get('delta', 0) > 0 else '#e74c3c'};">{latest.get('delta', 0):+.0f}</div>
            </div>
        </div>
        """, unsafe_allow_html=True)
        ...
        display_df = agg_df.tail(10).copy()  # Show last 10 records
        display_df['Time'] = display_df['timestamp'].dt.strftime('%H:%M')
        display_df['Price'] = display_df['close'].round(1)
        display_df['BI'] = display_df['buy_initiated'].astype(int)
        display_df['SI'] = display_df['sell_initiated'].astype(int)
        display_df['TΔ'] = display_df['tick_delta'].astype(int)
        display_df['CumΔ'] = display_df['cumulative_tick_delta'].astype(int)
        
        # Display compact table
        table_df = display_df[['Time', 'Price', 'BI', 'SI', 'TΔ', 'CumΔ']]
        ...
        <div class="market-stats">
            <h3 style="margin-top: 0; color: #2c3e50;">Market Stats</h3>
            <div style="text-align: center; padding: 40px; color: #6c757d;">No data available</div>
        </div>
        """, unsafe_allow_html=True)
