...
if not security_ids:
    st.error("❌ No security IDs found in historical data.")
    st.stop()
...
def fetch_live_data(security_id, retries=3, delay=5):
    """Fetch latest live data from Flask API with retries"""
    ...
    for attempt in range(1, retries + 1):
        try:
            ...
            st.warning(f"⚠️ Attempt {attempt}: Live API fetch failed ({e})")
            time.sleep(delay)
    return pd.DataFrame()
...
full_df = pd.concat([historical_df, live_df], ignore_index=True)\
...
if not agg_df.empty:
    ...
    st.warning("⚠️ Aggregated data is empty for this security.")
